...
class BoardRenderer:
    ...
    def draw(
        ...
        self._draw_captured_pieces(captured_pieces)
    ...
    def _draw_captured_pieces(self, captured_pieces):
        """Draw captured pieces on the right side of the board."""
        if not captured_pieces:
            return

        symbols = {
            0: {1: "♟", 2: "♜", 3: "♞", 4: "♝", 5: "♛", 6: "♚"}, # 白方的戰利品
            1: {1: "♙", 2: "♖", 3: "♘", 4: "♗", 5: "♕", 6: "♔"}  # 黑方的戰利品
        }

        x_base = cfg.BOARD_X + cfg.BOARD_PIXEL_W + 20

        for player in (0, 1):
            pieces = captured_pieces.get(player, [])
            if not pieces:
                continue

            pieces.sort(reverse=True)
            text = " ".join([symbols[player].get(p, "?") for p in pieces])

            surf, rect = self.label_font.render(text, fgcolor=(210, 210, 210), size=22)

            is_bottom = (player == 0 and not cfg.FLIPPED) or (player == 1 and cfg.FLIPPED)

            if is_bottom:
                y = cfg.BOARD_Y + cfg.BOARD_PIXEL_H - 130
            else:
                y = cfg.BOARD_Y + 260

            self.surface.blit(surf, (x_base, y))
